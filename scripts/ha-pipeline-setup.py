...
環境変数: HA_URL, WAKE_WORD, WHISPER_LANGUAGE, TTS_VOICE, OLLAMA_DEFAULT_MODEL
"""
...
TTS_VOICE     = os.environ.get("TTS_VOICE", "ja_JP-voicevox")
OLLAMA_MODEL  = os.environ.get("OLLAMA_DEFAULT_MODEL", "qwen2.5:3b")
...
def _discover_via_states():
    ...
    tts_e = next(
        (s["entity_id"] for s in states if s["entity_id"].startswith("tts.")
         and any(k in s["entity_id"] for k in ("voicevox", "piper", "wyoming"))),
        None,
    )
    ww_e = next(
        ...
        try:
            ...
            entity = next(
                (s["entity_id"] for s in states
                 if s["entity_id"].startswith("conversation.")
                 and "ollama" in s["entity_id"]),
                None,
            )
            if entity:
            ...
            pass
        time.sleep(5)
    print("  Ollama entity not found — using conversation.home_assistant")
...
def main():
    ...
    result = ws_call(sock, 20, "assist_pipeline/pipeline/create",
        ...
        tts_voice=TTS_VOICE if tts_entity else None,
